...
adapted for R. This is not a style guide. It's a guide to producing [readable,
reusable, and refactorable](https://github.com/ryanmcdermott/3rs-of-software-architecture)
software in R.
...
shaped into its final form. Finally, we chisel away the imperfections when we
review it with our peers. Don't beat yourself up for first drafts that need
...
This is not good because `brewery_name` can be `NULL`.

```r
create_microbrewery <- function(brewery_name = "Hipster Brew Co.") {
    ## ...
}
```
...
```r
create_microbrewery <- function(name = NULL) {
    if (is.null(name)) name <- "Hipster Brew Co."
    ## ...
}
```
